...
(function($){
	...
	$(document).on( 'preInit.dt', function ( e, settings ) 
	{
		...
		$(container).prepend(
			'<div id="' + filterWrapperId + '" class="row" align="right">' 
			+ filterDef.label + '</div>'
			);
	...
	$(document).on( 'init.dt', function ( e, settings ) 
	{
		...
		api.columns(filterDef.columnsIdxList).every( function () 
		{
			...
			{
				if (d != "") select.append( '<option value="' + d + '">' + d.substring(2) + '</option>' );
			} );
