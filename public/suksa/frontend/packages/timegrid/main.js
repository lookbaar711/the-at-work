    ...
    */
    var TimeGridEventRenderer = /** @class */ (function (_super) {
        ...
        TimeGridEventRenderer.prototype.renderSegHtml = function (seg, mirrorInfo) {
            ...
            var dt_split = fullTimeText.split(" - ");

            let time_array = [];
     
            for (let i = 0; i < 2; i++) {
                if (dt_split[i].substr(-2, 2) == 'AM') {
                    var dt_t = dt_split[i].substring(-3, 5);
                    var t = dt_t.split(":");
                    var h = 0;
                    if (t[0] < 10) {
                        h = '0' + t[0];
            ...
            let time = time_array[0] +' - '+ time_array[1];
    ...
    var TimeGrid = /** @class */ (function (_super) {
        ...
        TimeGrid.prototype.renderSlatRowHtml = function (dateProfile) {
            ...
            while (core.asRoughMs(slotTime) < core.asRoughMs(dateProfile.maxTime)) {
                ...
                if (isLabeled) {
                    var time = core.htmlEscape(dateEnv.format(slotDate, this.labelFormat));
                    if (time.substr(-2, 2) == 'am') {
                        if (time.substring(-1, 2) == 12) {
                            time = '00';
                        }else{
                            time = time.replace("am", "") * 1;
                            if(time < 10) time = '0' + time;
                        }
                    }else{
                        time = time.replace("pm", "") * 1;
                        if (time != 12) {
                            time = time*1 + 12;
                        }
                    }
